...
class ModelAnalyzer:
    ...
    def create_loss_plot(
        ...
        if len(train_losses) != len(val_losses):
            print(
                f"Warning - different length of lists - train: {len(train_losses)}, val: {len(val_losses)}"
            )
    ...
    def analyze_model(
        ...
        analysis = {
            ...
            "final_metrics": {"loss": final_loss, "accuracy": final_accuracy},
            "layer_statistics": {},
            "global_statistics": {},
        }
    ...
    def _create_visualizations(
        self, model: torch.nn.Module, analysis: Dict[str, Any], init_type: str
    ):
        ...
        axes[0, 0].axvline(
            ...
            color="red",
            linestyle="--",
            label=f"Średnia: {np.mean(all_weights):.4f}",
        )
        axes[0, 0].legend()

        # 2. Statystyki per warstwa
        layer_means = [
            analysis["layer_statistics"][name]["mean"]
            for name in analysis["layer_statistics"]
        ]
        layer_stds = [
            analysis["layer_statistics"][name]["std"]
            for name in analysis["layer_statistics"]
        ]
        ...
        axes[0, 1].set_xticklabels(
            [
                analysis["layer_statistics"][name]["layer_name"]
                for name in analysis["layer_statistics"]
            ],
            rotation=45,
        )
        ...
        layer_mins = [
            analysis["layer_statistics"][name]["min"]
            for name in analysis["layer_statistics"]
        ]
        layer_maxs = [
            analysis["layer_statistics"][name]["max"]
            for name in analysis["layer_statistics"]
        ]
    ...
    def compare_initializations(self, analysis_files: List[str]):
        ...
        for layer_idx, layer_name in enumerate(layer_names):
            ...
            x_pos = range(len(init_types))
            bars = axes[layer_idx].bar(
                x_pos, max_weights, alpha=0.7, color=plt.cm.tab10(layer_idx)
            )

            axes[layer_idx].set_title(
                f'{layer_name.split(".")[-2]}.{layer_name.split(".")[-1]}'
            )
    ...
    def _interpolate_models(modelA, modelB, alpha):
        ...
        with torch.no_grad():
            for pA, pB, pI in zip(
                modelA.parameters(), modelB.parameters(), model_interp.parameters()
            ):
