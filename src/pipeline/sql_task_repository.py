...
class SqlTaskRepository(TaskRepository):
    ...
    def get_by_executor_id(self, executor_task_id: str) -> TaskStatus | None:
        with self._db_cls() as db:
            cursor = db.execute(
                "SELECT task_id, task_status, webhook_token FROM tasks WHERE executor_task_id = %s",
                (executor_task_id,),
            )
            row = cursor.fetchone()
        if row is None:
            return None
        task_id, task_status, webhook_token = row
        return TaskStatus(task_id=str(task_id), task_status=str(task_status), webhook_token=webhook_token)
