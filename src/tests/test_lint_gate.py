...
_SCRIPT = pathlib.Path(__file__).resolve().parents[2] / "scripts" / "lint_baseline.py"
...
def test_violations_are_counted_per_file_and_code(monkeypatch):
    output = (
        "./src/backend/main.py:10:1: F401 unused\n"
        "./src/backend/main.py:12:1: F401 unused\n"
        "./src/backend/other.py:3:80: E501 too long\n"
    )
    monkeypatch.setattr(subprocess, "run", lambda *a, **k: _completed(1, output))
    assert lint_baseline.run_flake8() == {
        ("src/backend/main.py", "F401"): 2,
        ("src/backend/other.py", "E501"): 1,
    }
