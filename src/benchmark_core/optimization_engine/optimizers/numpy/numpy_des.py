...
class NumpyDES(NumpyBenchmarkOptimizer):
    ...
    def _expected_norm(dim: int) -> float:
        """
        E[||N(0, I_dim)||].
        Uses an asymptotic approximation for high dimensions to avoid
        overflow/NaN from gamma-ratio evaluation.
        """
        if dim <= 0:
    ...
    def _bounce_back_boundary(self, x_matrix: np.ndarray) -> np.ndarray:
        ...
        # Broadcast-safe boundary repair:
        # lower + abs(lower - x) % (upper - lower),
        # upper - abs(upper - x) % (upper - lower)
        repaired = np.where(
            repaired < lower_b,
            lower_b + np.mod(np.abs(lower_b - repaired), range_b),
            repaired,
        )
        repaired = np.where(
            repaired > upper_b,
            upper_b - np.mod(np.abs(upper_b - repaired), range_b),
            repaired,
        )
